...
pub fn draw_default_legends<'a>(
    ...
    // Default to other, e.i. the unmapped value we just added.
    ...
    let dropdown_index = dim.unwrap_or(&other_rank);
    if let Some(index) = widget::DropDownList::new(dimension_names.as_slice(), Some(*dropdown_index))
        ...
        .enumerate()
    {
        ...
        if index == 7usize {
            ...
            let dropdown_index = dim.unwrap_or(&other_rank);
            if let Some(index) =
                widget::DropDownList::new(dimension_names.as_slice(), Some(*dropdown_index))
                    .left_from(preview_id, 2.0f64)
                    .label_font_size(FONT_SIZE_SMALL)
                    .w(BUTTON_WIDTH)
                    .set(dropdown_id, &mut ui)
